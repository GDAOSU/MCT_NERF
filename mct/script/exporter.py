...
from nerfstudio.model_components.exporters import (
    generate_all_images,
    generate_point_cloud,
    generate_point_cloud_allimages,
)
from nerfstudio.utils.eval_utils import eval_all_images_setup, eval_setup
...
@dataclass
class ExportPointCloud_allimages(Exporter):
    """Export NeRF as a point cloud."""
    ...
    def main(self) -> None:
        ...
        _, pipeline, _ = eval_all_images_setup(self.load_config)

        # Increase the batchsize to speed up the evaluation.
        pipeline.datamanager.train_pixel_sampler.num_rays_per_batch = self.num_rays_per_batch

        pcd = generate_point_cloud_allimages(
            pipeline=pipeline,
...
class ExportImages(Exporter):
    ...
    def main(self) -> None:
        ...
        if not self.output_dir.exists():
            self.output_dir.mkdir(parents=True)
        _, pipeline, _ = eval_all_images_setup(self.load_config, eval_datapath=self.eval_datapath)

        # Increase the batchsize to speed up the evaluation.
        pipeline.datamanager.train_pixel_sampler.num_rays_per_batch = self.num_rays_per_batch
        pipeline.datamanager.eval_pixel_sampler.num_rays_per_batch = self.num_rays_per_batch

        pcd = generate_all_images(
            pipeline=pipeline,
            remove_outliers=True,
            outdir=self.output_dir,
            rgb_output_name="rgb",
...
Commands = Union[
    ...
    Annotated[ExportPointCloud_allimages, tyro.conf.subcommand(name="pointcloud_our")],
    Annotated[ExportTSDFMesh, tyro.conf.subcommand(name="tsdf")],
    Annotated[ExportPoissonMesh, tyro.conf.subcommand(name="poisson")],
    Annotated[ExportMarchingCubesMesh, tyro.conf.subcommand(name="marching-cubes")],
    Annotated[ExportImages, tyro.conf.subcommand(name="ExportImages")],
]
